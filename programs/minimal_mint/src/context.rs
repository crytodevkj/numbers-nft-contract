...
#[derive(Accounts)]
#[instruction(bump: u8)]
pub struct CreateNftPool<'info> {
    #[account(init, seeds = [b"mint pda".as_ref(), "0".as_bytes()], bump, payer = user, space = 8 + 32 + 8)]
    pub pool: Account<'info, MintPda>,
...
pub struct MintNFT<'info> {
    ...
    #[account(mut)]
    /// CHECK: This is not dangerous because we don't read or write from this account
    pub authority1: AccountInfo<'info>,
    /// CHECK: This is not dangerous because we don't read or write from this account
    pub authority2: AccountInfo<'info>,
    ...
    #[account(mut)]
    pub mint_pda: Account<'info, MintPda>,
...
pub struct InitializeCandyMachine<'info> {
    #[account(
        ...
        space =
            8  +  // discriminator
                  // \/ candy_machine
            8  + 8 + 8 + (40 * 2 /* multiply by n of creators */) + 8 + 2 + 9 + 3333 * 130 +
            32 +  // authority
            32 +  // start date
            1,   // bump + bonus
        bump,
